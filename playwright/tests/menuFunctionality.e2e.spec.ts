...
test.describe('Menu button navigation', () => {
  ...
  test('WHEN user clicks Home, THEN redirected to login page', async ({ page }) => {
    ...
    await test.step('THEN: I are redirected to the login page', async () => {
      await expect(page).toHaveURL('/');
    });
  });

  test('WHEN user clicks About, THEN Agile Actors page opens in new tab', async ({ page, context }) => {
    let newTab;

    await test.step('WHEN: I click the About button', async () => {
