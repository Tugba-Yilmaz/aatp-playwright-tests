...
test.describe('Checkout process', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await test.step('GIVEN: The user has added products to teh cart', async () => {
      ...
      (await cartPage.openCart(), await cartPage.goToCheckout());
    });
  });

  test(' Successful checkout', async () => {
    await test.step('WHEN:  The user complete the checkout process with valid info', async () => {
  ...
  test(' Cancel order during checkout', async () => {
    await test.step('WHEN: The user choose to cancel the order from the checkout page', async () => {
